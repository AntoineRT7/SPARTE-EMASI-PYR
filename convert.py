...
# Valeurs de la colonne « Nom-Prénom » qui ne sont PAS de vraies personnes -> masquées
HIDE_NAME = re.compile(r"^(en attente d.affectation|annul(é|e)|report(é|e)|informations diverses)", re.I)
...
def main():
    ...
    for r in rows[hr + 1:]:
        ...
        date = to_dt(g(r, "deb"))
        if date is None:
            # Titre de section : dans la colonne Nom OU la colonne Ressource selon le format,
            # à condition qu'aucune colonne descriptive ne soit remplie.
            ress = cell(g(r, "ress"))
            descr = cell(g(r, "lib")) or cell(g(r, "com")) or cell(g(r, "type"))
            title = nom if (nom and nom.lower() != "nom-prénom") else ress
            if title and not descr:
                section = title
            continue
        if not nom or nom.lower() == "nom-prénom":
            continue
        if HIDE_NAME.match(nom):       # statut (non assigné, annulé, reporté…) : masqué
            continue
        fin = to_dt(g(r, "fin"))
        out.append({
            "nom": nom,
